...
export default function NotificationsTab() {
  ...
  if (loading) return <p className="text-neutral-400 p-4">Loading notifications...</p>
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {['ALL', 'UNREAD', 'FLIGHT', 'MAINTENANCE', 'BAGGAGE', 'GENERAL'].map(f => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${filter === f ? 'bg-blue-600 text-white border-blue-600' : 'bg-neutral-900 text-neutral-300 border-gray-300 hover:border-blue-400'}`}
          >
      ...
      <div className="space-y-3">
        {filtered.length === 0 && (
          <div className="text-center text-neutral-500 py-10">No notifications found.</div>
        )}
        {filtered.map(n => (
          <div key={n.id} className={`flex items-start gap-4 p-4 rounded-lg border ${n.is_read ? 'bg-neutral-900 border-neutral-700' : 'bg-blue-50 border-blue-200'}`}>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${TYPE_COLORS[n.type] ?? 'bg-neutral-800 text-neutral-300'}`}>
                  {TYPE_LABELS[n.type] ?? n.type}
                </span>
                {!n.is_read && <span className="w-2 h-2 bg-blue-500 rounded-full inline-block" title="Unread" />}
              </div>
              <p className="text-sm text-white">{n.message}</p>
              <p className="text-xs text-neutral-500 mt-1">{new Date(n.created_at).toLocaleString()}</p>
            </div>
