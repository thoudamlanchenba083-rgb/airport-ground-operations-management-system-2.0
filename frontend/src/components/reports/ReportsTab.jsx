...
export default function ReportsTab() {
  ...
  if (loading) return <p className="text-gray-500 p-4">Loading reports...</p>
  ...
  return (
    <div>
      {/* Summary Cards */}
      <div className="mb-6">
        <h3 className="text-base font-semibold text-gray-700 mb-3">Live Summaries</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {['FLIGHT', 'BAGGAGE', 'MAINTENANCE', 'STAFF'].map(type => (
            <button
              key={type}
              onClick={() => fetchSummary(type)}
              className={`rounded-lg border p-3 text-left hover:shadow transition-shadow ${summaryType === type ? 'border-blue-500 bg-blue-50' : 'bg-white border-gray-200'}`}
            >
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${TYPE_COLORS[type]}`}>
                {REPORT_TYPES.find(r => r.value === type)?.label}
              </span>
              <p className="text-xs text-gray-500 mt-2">Click to load summary</p>
            </button>
          ))}
        </div>

        {loadingSummary && <p className="text-sm text-gray-400 mt-3">Loading summary...</p>}
        {summary && !loadingSummary && (
          <div className="mt-3 bg-gray-50 border rounded-lg p-4">
            <h4 className="text-sm font-semibold text-gray-700 mb-2">{summaryType} Summary</h4>
            {summary.error
              ? <p className="text-red-500 text-sm">{summary.error}</p>
              : <pre className="text-xs text-gray-600 whitespace-pre-wrap">{JSON.stringify(summary, null, 2)}</pre>
            }
          </div>
        )}
      </div>
      ...
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-base font-semibold text-gray-700">Saved Reports</h3>
      ...
      <div className="space-y-3">
        {reports.length === 0 && <p className="text-center text-gray-400 py-8">No reports yet.</p>}
        {reports.map(r => (
          <div key={r.id} className="bg-white border rounded-lg p-4 flex justify-between items-start gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${TYPE_COLORS[r.report_type] ?? 'bg-gray-100 text-gray-600'}`}>
