...
class IncidentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        occurred_at = data.get('occurred_at', getattr(self.instance, 'occurred_at', None))
        ...
        if occurred_at and resolved_at and resolved_at < occurred_at:
            raise serializers.ValidationError('resolved_at cannot be before occurred_at.')

        status = data.get('status', getattr(self.instance, 'status', None))
        if status in ('RESOLVED', 'CLOSED') and not resolved_at:
            raise serializers.ValidationError('resolved_at is required when status is Resolved or Closed.')
        return data