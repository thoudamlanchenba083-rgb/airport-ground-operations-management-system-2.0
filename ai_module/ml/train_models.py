...
def train_delay_model():
    ...
    if has_enough_real_delay_data():
        print(
            f"  using REAL data ({count_real_delay_records()} departed flights)")
        df = extract_real_flight_delay_data()
    else:
        print(
            f"  only {count_real_delay_records()}/{MIN_REAL_DELAY_RECORDS} real records - using SYNTHETIC data")
    ...
    reg.fit(Xtr, ytr)
    print(
        f"  delay_minutes regressor MAE: {mean_absolute_error(yte, reg.predict(Xte)):.2f} min")
    ...
    clf.fit(Xtr, ytr)
    print(
        f"  is_delayed classifier accuracy: {accuracy_score(yte, clf.predict(Xte)):.3f}")
...
def train_maintenance_model():
    ...
    if has_enough_real_maintenance_data():
        print(
            f"  using REAL data ({count_real_maintenance_records()} maintenance requests)")
        df = extract_real_maintenance_data()
    else:
        print(
            f"  only {count_real_maintenance_records()}/{MIN_REAL_MAINTENANCE_RECORDS} real records - using SYNTHETIC data")
        df = generate_maintenance_dataset()

    features = [
        'flight_hours_since_service',
        'cycles_since_service',
        'aircraft_age_years',
        'reported_faults_last_30d',
        'avg_temp_exposure_c',
        'component_wear_index',
    ]
    X = df[features]
    ...
    reg.fit(Xtr, ytr)
    print(
        f"  urgency_score regressor R2: {r2_score(yte, reg.predict(Xte)):.3f}")
    ...
    clf.fit(Xtr, ytr)
    print(
        f"  maintenance_required classifier accuracy: {accuracy_score(yte, clf.predict(Xte)):.3f}")
...
def train_weather_risk_model():
    ...
    reg.fit(Xtr, ytr)
    print(
        f"  weather_risk_score regressor R2: {r2_score(yte, reg.predict(Xte)):.3f}")
    ...
    clf.fit(Xtr, ytr)
    print(
        f"  delay_likely classifier accuracy: {accuracy_score(yte, clf.predict(Xte)):.3f}")
...
def train_staff_model():
    ...
    for target_col, filename in targets.items():
        ...
        reg.fit(Xtr, ytr)
        print(
            f"  {target_col} regressor MAE: {mean_absolute_error(yte, reg.predict(Xte)):.2f}")
        _save(reg, filename)
...
def train_gate_model():
    ...
    reg.fit(Xtr, ytr)
    print(
        f"  suitability_score regressor R2: {r2_score(yte, reg.predict(Xte)):.3f}")
...
def train_equipment_model():
    ...
    reg.fit(Xtr, ytr)
    print(
        f"  failure_risk_score regressor R2: {r2_score(yte, reg.predict(Xte)):.3f}")
