...
def _call_gemini(contents):
    ...
    resp = requests.post(
        ...
        params={"key": api_key},
        json={"system_instruction": {"parts": [{"text": SYSTEM_PROMPT}]}, "contents": contents, "tools": GEMINI_TOOLS},
        timeout=30,
    )
...
def get_reply(message, user=None, session_id=''):
    ...
    contents = [{"role": "model" if role == "assistant" else "user", "parts": [{"text": content}]}
                for role, content in history] or [{"role": "user", "parts": [{"text": message}]}]
    ...
    for _ in range(MAX_TOOL_ROUNDTRIPS):
        ...
        contents.append({"role": "user", "parts": [
            {"functionResponse": {"name": fc["name"], "response": {"result": run_tool(fc["name"], fc.get("args"))}}}
            for fc in calls
        ]})
