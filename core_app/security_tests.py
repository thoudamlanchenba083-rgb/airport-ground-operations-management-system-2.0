...
class RoleBasedAuthorizationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.admin = User.objects.create_user(
            username='sec_admin', password='TestPass123!', role='ADMIN', is_staff=True
        )
    ...
    def test_regular_user_cannot_create_airline(self):
        ...
        response = self.client.post('/api/flights/airlines/', {'name': 'Test Air', 'code': 'TA1'})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_admin_can_create_airline(self):
        token = self._login('sec_admin', 'TestPass123!')
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {token}')
        response = self.client.post('/api/flights/airlines/', {'name': 'Test Air', 'code': 'TA2'})
        self.assertIn(response.status_code, [status.HTTP_200_OK, status.HTTP_201_CREATED])

    def test_wrong_password_rejected(self):
        response = self.client.post('/api/token/', {'username': 'sec_user', 'password': 'WrongPassword'})
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
class InformationDisclosureTests(TestCase):
    ...
    def test_404_does_not_leak_debug_traceback(self):
        ...
        # Django's debug error page includes this literal string when DEBUG=True
        self.assertNotIn('django version', body)
        self.assertNotIn('traceback', body)

    def test_malformed_request_does_not_leak_secret_key(self):
        response = self.client.post('/api/flights/flights/', {'not_a_real_field': '<script>alert(1)</script>'})
...
class RateLimitTests(TestCase):
    ...
    def setUp(self):
        self.client = APIClient()
        User.objects.create_user(username='ratelimit_user', password='TestPass123!')

    def test_repeated_failed_logins_are_rate_limited(self):
        responses = []
        for _ in range(7):
            response = self.client.post('/api/token/', {'username': 'ratelimit_user', 'password': 'wrong'})
            responses.append(response.status_code)
        # django-ratelimit blocks with 403 (PermissionDenied) by default, not 429
