    ...
    def validate(self, data):
        ...
        max_weight = data.get(
            'max_weight_kg', getattr(
                self.instance, 'max_weight_kg', 0))
        if max_weight and weight and weight > max_weight:
            raise serializers.ValidationError(
                'weight_kg cannot exceed max_weight_kg.')
        return data
...
class CargoItemSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if is_dg and not dg_class:
            raise serializers.ValidationError(
                'dangerous_goods_class is required when is_dangerous_goods is True.')
        if not is_dg and dg_class:
            raise serializers.ValidationError(
                'dangerous_goods_class must be empty when is_dangerous_goods is False.')
        return data
