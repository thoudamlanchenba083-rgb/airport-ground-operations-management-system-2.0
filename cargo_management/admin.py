...
class ULDAdmin(admin.ModelAdmin):
    list_display = ['uld_id', 'uld_type', 'status', 'flight', 'position', 'weight_kg', 'max_weight_kg']
...
class CargoManifestAdmin(admin.ModelAdmin):
    list_display = ['manifest_number', 'flight', 'total_weight_kg', 'is_finalized', 'created_at']
...
class CargoItemAdmin(admin.ModelAdmin):
    list_display = ['awb_number', 'description', 'manifest', 'uld', 'weight_kg', 'status', 'is_dangerous_goods', 'dangerous_goods_class']
