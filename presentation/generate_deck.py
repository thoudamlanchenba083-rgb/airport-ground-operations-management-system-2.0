...
def _title_bar(slide, title):
    ...
    line = slide.shapes.add_shape(
        1, Inches(0.6), Inches(1.15), Inches(6.5), Pt(3))
    line.fill.solid()
...
def add_bullet_slide(title, bullets, note=None):
    ...
    if note:
        nb = slide.shapes.add_textbox(
            Inches(0.8), Inches(6.7), Inches(11.5), Inches(0.6))
...
def add_two_col_slide(
    ...
    def col(x, heading, bullets):
        hb = slide.shapes.add_textbox(
            Inches(x), Inches(1.6), Inches(5.6), Inches(0.5))
        hp = hb.text_frame.paragraphs[0]
        hp.text = heading
        hp.font.size = Pt(20)
        hp.font.bold = True
        hp.font.color.rgb = SKY
        bb = slide.shapes.add_textbox(
            Inches(x), Inches(2.15), Inches(5.6), Inches(4.8))
        tf = bb.text_frame
        tf.word_wrap = True
        for i, b in enumerate(bullets):
            p = tf.paragraphs[0] if i == 0 else tf.add_paragraph()
...
def add_status_slide(title, rows):
    ...
    for item, status, color in rows:
        box = slide.shapes.add_textbox(
            Inches(0.8), Inches(y), Inches(9.5), Inches(0.55))
        p = box.text_frame.paragraphs[0]
        p.text = item
        p.font.size = Pt(18)
        p.font.color.rgb = NAVY
        chip = slide.shapes.add_shape(
            1, Inches(10.5), Inches(y), Inches(2.0), Inches(0.42))
        chip.fill.solid()
...
arrow_note = slide.shapes.add_textbox(
    Inches(3.85), Inches(2.0), Inches(1.1), Inches(0.7))
...
np_.paragraphs[0].font.size = Pt(15)
np_.paragraphs[0].font.color.rgb = GREY
# ---------- SLIDE 17: UAT ----------
